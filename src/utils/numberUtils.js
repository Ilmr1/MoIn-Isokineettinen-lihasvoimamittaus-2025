...
export const padRoundDecimalsToLength = (
  number,
  charactersCount,
  maxDecimalsToKeep,
) => {
  const decimals = String(Math.trunc(Math.abs(number))).length;
  const decimalsToKeep = min(
    Math.max(0, charactersCount - decimals),
    maxDecimalsToKeep,
  );
...
export const padTrucateDecimalsToLength = (
  number,
  charactersCount,
  maxDecimalsToKeep,
) => {
...
export const absDelta = (start, end) =>
  Math.abs(Math.max(end, start) - Math.min(end, start));
export const min = (...nums) =>
  nums.reduce((acc, num) => Math.min(acc ?? num, num ?? acc));
export const max = (...nums) =>
  nums.reduce((acc, num) => Math.max(acc ?? num, num ?? acc));
export const middle = (a, b) =>
  Math.min(a, b) + (Math.max(a, b) - Math.min(a, b)) / 2;
...
export const ceilFurthestFromValue = (num, val = 0) =>
  num > val ? Math.ceil(num) : Math.floor(num);
export const floorClosestToValue = (num, val = 0) =>
  num > val ? Math.floor(num) : Math.ceil(num);
