...
export function SpeedChart(props) {
  ...
  const flipY = y => Math.abs(y - props.parsedCTM.minmax.maxSpeed);
  ...
  createRenderEffect(() => {
    ...
    const totalDataHeight = props.parsedCTM.minmax.maxSpeed - props.parsedCTM.minmax.minSpeed;
    ...
    for (let x = 0; x < props.parsedCTM.data.length; x++) {
      const y = props.parsedCTM.data[x][1];
