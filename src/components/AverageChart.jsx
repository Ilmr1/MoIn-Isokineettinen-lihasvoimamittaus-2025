...
function Chart(props) {
  ...
  function AngleSpecificHQRatio(props) {
    ...
    const combinedValues = createMemo(() => {
      ...
      const endAngles = [];
      for (const { rawObject } of files) {
        startAngles.push(rawObject.pointCollections.angleSpecificHQRatio.maxAngle);
        endAngles.push(rawObject.pointCollections.angleSpecificHQRatio.minAngle);
      }
      ...
      return {
        minValue: Math.min(...files.map(parsedData => parsedData.rawObject.pointCollections.angleSpecificHQRatio.minValue)),
        maxValue: Math.max(...files.map(parsedData => parsedData.rawObject.pointCollections.angleSpecificHQRatio.maxValue)),
        xStartValue,
        xEndValue,
      }
    });
    ...
    return (
      <Show when={combinedValues().maxValue}>
        <svg width={svgArea.width} height={svgArea.height} onMouseLeave={clearHoverCoors} onMouseMove={updateHoverCoords}>
          <ChartPadding name="border" {...svgArea} paddingLeft={80} paddingRight={50} paddingBottom={40} paddingTop={22}>{borderArea => (
            <>
              <ChartTextTop {...borderArea} title="Angle Specific HQ-ratio" />
              <ChartBorder {...borderArea} />
              <ChartPadding name="lines" {...borderArea} padding={15}>{lineArea => (
                <>
                  <ChartGridAlignedWithFloorXAxisLabels
                  ...
                  <g data-lines>
                    <For each={props.listOfParsedCTM()}>{parsedData => (
                      <>
                        <ChartPath
                          points={parsedData.rawObject.pointCollections.angleSpecificHQRatio.points}
                          splits={parsedData.rawObject.splitCollections.angleSpecificHQRatio.splits}
                          startIndex={parsedData.rawObject.splitCollections.angleSpecificHQRatio.startIndex}
                          endIndex={parsedData.rawObject.splitCollections.angleSpecificHQRatio.endIndex}
                          stroke={parsedData.baseColor}
                  ...
                  <ChartMousePositionInPercentage {...controls} {...borderArea} width={lineArea.width} x={lineArea.x}>{mouseArea => (
                    <>
                      ...
                      <ChartHoverToolTip
                        ...
                        x={borderArea.x}
                        xWithPadding={lineArea.x}
                        listOfPoints={props.listOfParsedCTM().map(v => v.rawObject.pointCollections.angleSpecificHQRatio.points)}
                        listOfSplits={props.listOfParsedCTM().map(v => v.rawObject.splitCollections.angleSpecificHQRatio)}
                        maxValue={combinedValues().maxValue}
                        minValue={combinedValues().minValue}
                        colors={colors()}
                      />
